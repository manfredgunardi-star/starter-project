...
- **A whole-file hash comparison against a fenced code block embedded in this plan (or in a task brief generated from it) will report a false mismatch even when content is character-for-character identical — normalize before comparing.** This is a distinct case from the byte-safe comparison bullet above, which covers verifying two reads of the *same* file/document are identical. Here the two things being compared are structurally different: a materialized source file conventionally ends with a trailing newline (standard convention this repo's lint config expects — do not strip it to force a hash match), while a markdown fenced code block (` ```jsx ... ``` `) has no such trailing newline at its own boundary. Hashing the whole file's raw bytes against the raw block text will therefore differ by exactly one trailing-newline byte even when every other character matches, which is a false failure of the verification method, not a real content defect — confirmed in this plan's Task 2 round 2 (`JournalLinesEditor.jsx`/`ManualJournalFormPage.jsx` both matched their spec blocks character-for-character once this was accounted for). To verify a materialized file against a plan/brief code block correctly: either normalize both sides first (strip a single trailing newline and normalize CRLF to LF on the file's content before hashing) or skip hashing and compare content equality directly (e.g., in Node: `expected.trimEnd() === actual.replace(/\r\n/g,'\n').trimEnd()`). Reserve raw whole-file `Get-FileHash` comparison for its original purpose — two reads of the same file/document — not for comparing a file against its markdown source block.
- **Prefer `git -C "<path>" <command>` over `cd "<path>"` followed by a separate `git ...` line, for git specifically:** every command block in this plan pairs a `cd "..."` line with subsequent commands assuming the working directory persists across them, which has worked reliably for `npm run lint`/`npm run build`/`rg` throughout this plan's execution so far — but if a `git add`/`git commit` ever fails with a working-directory-resolution error (e.g. Windows error 267, "The directory name is invalid") even though the same path has worked in prior steps, do not assume the path is wrong before checking: retry that specific git command using `git -C "<the same absolute path>" add ...` / `git -C "<the same absolute path>" commit -m "..."` instead, which passes the path explicitly to git and does not depend on any prior `cd` having taken effect. This is the same pattern already used successfully for the initial `git -C "C:\Project" worktree add ...` step.
...
If Step 4's live browser regression was skipped because no `.env` was present in the worktree (the expected case, per this plan's Task 1 Step 3 / Task 3 Step 3 "defer if no live DB" pattern), use this commit message, which adds one paragraph noting the deferral — this is not a deviation from the plan, it is the required disclosure when a verification step could not run:
...
Live manual regression (Step 4) deferred: no .env present in this
worktree, so the dev server / browser check could not be run.

Co-Authored-By: Claude Sonnet 5 <noreply@anthropic.com>"
```

If `.env` was present and Step 4's full browser regression checklist was actually completed, omit that middle paragraph and use only the original two-paragraph message:
...
**Revision note 10 (working-directory error on `git commit`, root cause unconfirmed):** immediately after revision note 9's conclusion, the `git commit` for Task 1 Step 4 failed with a Windows IO error (`code 267, NotADirectory, "The directory name is invalid"`) on the same worktree path (`C:\Project\.worktrees\erp-acc\non-ap-ar-cash-transaction\apps\erp-acc\erp-app`) that every prior step in this plan — `git add`, `npm run lint`, `npm run build`, multiple `rg` searches — had already used successfully. Claude independently verified from outside Codex's environment that the path exists, is a proper directory, contains the expected project files, and that `git worktree list` shows the worktree still correctly registered at `HEAD c7d46ca` with the branch intact — so the path itself is not wrong. Unlike revision notes 1-9, this one's root cause could not be conclusively identified (most likely a transient issue in the command-execution path for this specific invocation, not a plan or content defect) — it is recorded here rather than confidently classified. Mitigated, not "fixed": added a Global Constraints note recommending `git -C "<path>" <command>` (explicit path, no dependency on a prior `cd` having taken effect) as a more robust retry pattern for any git command that fails this way, matching the pattern already used successfully for the initial `git worktree add` step.
