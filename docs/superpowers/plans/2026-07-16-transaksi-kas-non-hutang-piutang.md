...
- [ ] The initial `select * into v_journal from journals where id = p_journal_id for update;` uses `for update` — this locks the row so a second concurrent call on the same `p_journal_id` blocks until the first transaction commits, matching the locking idiom `execute_asset_disposal` already uses: originally defined in `supabase/migrations/014_fixed_assets.sql` (function starts around line 449, its `select ... for update` around line 480), re-created with an added period-lock guard — same `for update` lock kept — in `supabase/migrations/016_period_lock_enforcement.sql` (function starts around line 632, its `select ... for update` around line 665; this is the currently-active definition, since it's the later migration). Open `016_period_lock_enforcement.sql` and confirm the `select * into v_asset from assets where id = p_asset_id for update;` line yourself before treating this as verified — do not take this citation on faith, the plan has previously cited the wrong file for this same function.
...
For the guard-calling pattern (first checklist item below), compare against `016_period_lock_enforcement.sql`'s versions of `post_transfer`/`post_expense`, not `011`'s originals — `011`'s versions do not have the guards and are not a valid reference for that specific check. For structural details unrelated to guards (SQL body shape, `generate_number` usage, the account-balance formula), either file is equivalent since `016` only added the two `perform` guard calls and changed nothing else in those two functions' bodies.
...
- [ ] Declared `language plpgsql security definer set search_path = public` (matches the trailing `$$ language plpgsql security definer set search_path = public;` form used by `post_transfer`/`post_expense` in either `011_posting_functions.sql` or `016_period_lock_enforcement.sql` — this part of their body is identical between the two files — or the `language plpgsql / security definer / set search_path = public` block form used by `create_asset_acquisition_journal` in `016_period_lock_enforcement.sql` line 491 — both forms are equivalent, this migration uses the block form).
...
- [ ] **New COA codes don't collide, and parent codes exist** (`coa.code` is `text not null unique` per `002_master_data.sql:76` — a duplicate code makes the migration fail at apply-time; a missing parent code silently leaves `parent_id` null instead of erroring, since `parent_id` is nullable). Run exactly this (already verified to produce the expected result below — do not modify the argument list, in particular do not turn `supabase/migrations` into a glob like `supabase/migrations/*.sql`, which fails on this repo's PowerShell shell with `os error 123` because the shell does not expand the wildcard before handing it to the search tool; passing the bare directory instead lets the tool recurse into it on any shell):

```bash
cd "C:\Project\.worktrees\erp-acc\non-ap-ar-cash-transaction\apps\erp-acc\erp-app"
rg -n "'1-17000'|'5-20000'|'1-10000'|'5-00000'" supabase/seed.sql supabase/migrations
```
Expected output: exactly 5 matching lines, all in `supabase/seed.sql` (none in any file under `supabase/migrations`) — `1-10000` appears on 3 of those lines and `5-00000` on the other 2 (confirming both parent codes already exist to attach the new accounts to), and `1-17000`/`5-20000` appear on **zero** lines (confirming both new codes are unused anywhere in `seed.sql` or any existing migration, so the `unique` constraint on `coa.code` won't reject them). If the line/match count differs from this, STOP and report the actual output — do not proceed with the migration. If `rg` is not available in your shell, use `Select-String -Path supabase\seed.sql,supabase\migrations\*.sql -Pattern "1-17000|5-20000|1-10000|5-00000"` instead (PowerShell's own `Select-String` expands `*.sql` correctly, unlike passing the same glob to an external executable like `rg`).
