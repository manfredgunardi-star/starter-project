...
export default function SalesReportPage() {
  ...
  const exportPDF = () => {
    ...
    doc.autoTable({
      startY: 32,
      head: [['No. Invoice', 'Tanggal', 'Customer', 'Subtotal', 'PPN', 'Total', 'Terbayar', 'Sisa', 'Status']],
      body: data.map(row => {
  ...
  const exportExcel = () => {
    const rows = [
      ...
      [],
      ['No. Invoice', 'Tanggal', 'Customer', 'Subtotal', 'PPN', 'Total', 'Terbayar', 'Sisa', 'Status'],
      ...data.map(row => {
        const outstanding = Number(row.total) - Number(row.amount_paid)
        const status = STATUS_MAP[row.status]?.label || row.status
        return [
    ...
    XLSX.writeFile(wb, `laporan-penjualan-${startDate}-${endDate}.xlsx`)
  }
      ...
      {data && !loading && (
        <>
          ...
          <Row gutter={16}>
            ...
            <Col xs={12} sm={5}>
              <Statistic title="Piutang" value={formatCurrency(totalOutstanding)} />
            </Col>
          </Row>
