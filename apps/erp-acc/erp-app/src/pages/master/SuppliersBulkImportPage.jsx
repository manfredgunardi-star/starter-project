...
import { createSupplier } from '../../services/masterDataService'
...
function downloadTemplate() {
  const ws = XLSX.utils.aoa_to_sheet([
    ...
    ['CV Batu Alam', 'Jl. Industri No. 5, Bandung', '02212345678', 'info@batualam.com', '02.345.678.9-012.000'],
  ])
  ws['!cols'] = [{ wch: 30 }, { wch: 35 }, { wch: 18 }, { wch: 30 }, { wch: 25 }]
...
function parseRows(jsonRows) {
  return jsonRows.map((r, i) => {
    ...
    return {
      ...
      errors,
      data: { name, address: address || null, phone: phone || null, email: email || null, npwp: npwp || null },
    }
  })
}
...
export default function SuppliersBulkImportPage() {
  ...
  async function handleImport() {
    ...
    // Prefetch nama supplier yang sudah ada untuk dedup
    const namesToCheck = validRows.map(r => r.data.name)
    ...
    for (let i = 0; i < validRows.length; i++) {
      ...
      if (existingNames.has(row.data.name)) {
        skipped++
      } else {
        try {
          await createSupplier(row.data)
          success++
        } catch (err) {
  ...
  return (
    <Space direction="vertical" style={{ width: '100%', padding: 24 }}>
      ...
      >
        <ArrowLeft size={18} /> Kembali ke Daftar Supplier
      </button>

      <Flex justify="space-between" align="center">
        <Typography.Title level={4} style={{ margin: 0 }}>Bulk Import Supplier</Typography.Title>
        <button
      ...
      <Card title={<Typography.Text strong>Upload File Excel</Typography.Text>}>
        <Typography.Text type="secondary">
          Download template di atas, isi data supplier, lalu upload file .xlsx.
        </Typography.Text>
        <div
          ...
          <div style={{ overflowX: 'auto' }}>
            <table style={{ minWidth: '100%', fontSize: 12, borderCollapse: 'collapse' }}>
              <thead style={{ backgroundColor: '#f9fafb', borderBottom: '1px solid #e5e7eb' }}>
                <tr>
                  {['#', 'Nama', 'Alamat', 'Telepon', 'Email', 'NPWP', 'Status'].map(h => (
              ...
              <tbody>
                {rows.map(row => (
                  <tr key={row.rowNum} style={{ borderBottom: '1px solid #f3f4f6', backgroundColor: row.valid ? 'transparent' : '#fef2f2' }}>
                    ...
                    <td style={{ padding: '8px 12px', color: '#374151' }}>{row.data.address || '—'}</td>
                    <td style={{ padding: '8px 12px', color: '#374151' }}>{row.data.phone || '—'}</td>
                    <td style={{ padding: '8px 12px', color: '#374151' }}>{row.data.email || '—'}</td>
                    <td style={{ padding: '8px 12px', color: '#374151' }}>{row.data.npwp || '—'}</td>
      ...
      {validRows.length > 0 && !summary && (
        <Card>
          ...
          >
            {importing ? `Mengimpor... (${progress}%)` : `Import ${validRows.length} Supplier Valid`}
          </button>
        </Card>
      )}
            ...
            <Col span={8}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 12, padding: 16, backgroundColor: '#fef9c3', borderRadius: 8 }}>
                ...
                <div>
                  ...
                  <div style={{ fontSize: 14, color: '#ca8a04' }}>Dilewati (duplikat nama)</div>
                </div>
              </div>
            </Col>
            <Col span={8}>
