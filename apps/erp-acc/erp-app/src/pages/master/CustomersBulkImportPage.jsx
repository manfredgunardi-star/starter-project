...
import { createCustomer } from '../../services/masterDataService'
...
function downloadTemplate() {
  const ws = XLSX.utils.aoa_to_sheet([
    ...
    ['PT Maju Jaya', 'Jl. Sudirman No. 1, Jakarta', '08123456789', 'info@majujaya.com', '01.234.567.8-901.000'],
  ])
...
export default function CustomersBulkImportPage() {
  ...
  async function handleImport() {
    ...
    // Prefetch nama customer yang sudah ada untuk dedup
  ...
  return (
    <Space direction="vertical" style={{ width: '100%', padding: 24 }}>
      ...
      >
        <ArrowLeft size={18} /> Kembali ke Daftar Customer
      </button>

      <Flex justify="space-between" align="center">
        <Typography.Title level={4} style={{ margin: 0 }}>Bulk Import Customer</Typography.Title>
        <button
      ...
      <Card title={<Typography.Text strong>Upload File Excel</Typography.Text>}>
        <Typography.Text type="secondary">
          Download template di atas, isi data customer, lalu upload file .xlsx.
        </Typography.Text>
        <div
      ...
      {validRows.length > 0 && !summary && (
        <Card>
          ...
          >
            {importing ? `Mengimpor... (${progress}%)` : `Import ${validRows.length} Customer Valid`}
          </button>
        </Card>
      )}
