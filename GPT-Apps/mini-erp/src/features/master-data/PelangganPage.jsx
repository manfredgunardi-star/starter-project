...
function PelangganForm({ data, onChange }) {
  return (
    <div className="space-y-5">
      ...
      <div className="grid gap-4 sm:grid-cols-2">
        <TextField
          label="Telepon"
          onChange={(event) => onChange({ telepon: event.target.value })}
...
export function PelangganPage() {
  const { items, loading, error, save, remove, restore } = useMasterData('pelanggan', { prefix: 'pel' });
  ...
  const filteredItems = useMemo(() => {
    ...
    return items.filter((item) => {
      ...
      const matchQuery =
        !normalizedQuery ||
        [item.kode, item.nama, item.telepon, item.email, item.alamat, item.npwp]
          .filter(Boolean)
  ...
  async function handleSubmit(event) {
    ...
    try {
      await save({
        ...form,
        kode: form.kode.trim() || `CUST-${String(items.length + 1).padStart(3, '0')}`,
        nama: form.nama.trim(),
  ...
  async function handleDelete() {
    if (!form.id) return;
    const confirmed = window.confirm(`Nonaktifkan pelanggan "${form.nama}"?`);
    if (!confirmed) return;
  ...
  return (
    <div>
      <PageHeader
        eyebrow="Master Data"
        title="Pelanggan"
        description="Kelola pelanggan dengan soft delete, audit trail, dan company scope sejak awal."
        actions={<Button icon={Plus} onClick={openCreateDrawer}>Tambah Pelanggan</Button>}
      />

      <div className="mb-4 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <SearchInput onChange={setQuery} placeholder="Cari kode, nama, telepon, atau email" value={query} />
      ...
      {loading ? (
        ...
        <EmptyState
          ...
          description="Tambahkan pelanggan pertama untuk mulai mengisi master data company ini."
          action={<Button icon={Plus} onClick={openCreateDrawer}>Tambah Pelanggan</Button>}
        />
      )}

      <Drawer
        description={form.id ? 'Edit detail pelanggan dan simpan perubahan.' : 'Tambah pelanggan baru ke master data company aktif.'}
        ...
        footer={
          <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            ...
            <div className="flex gap-2 sm:justify-end">
              ...
              <Button form="pelanggan-form" type="submit" disabled={saving || !form.nama.trim()}>
      ...
      >
        <form id="pelanggan-form" onSubmit={handleSubmit}>
          <PelangganForm data={form} onChange={updateForm} />
        </form>
      </Drawer>
    </div>
  );
