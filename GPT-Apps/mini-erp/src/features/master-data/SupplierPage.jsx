...
function SupplierForm({ data, onChange }) {
  return (
...
export function SupplierPage() {
  const { items, loading, error, save, remove, restore } = useMasterData('supplier', { prefix: 'sup' });
  ...
  async function handleSubmit(event) {
    ...
    try {
      await save({
        ...form,
        kode: form.kode.trim() || `SUP-${String(items.length + 1).padStart(3, '0')}`,
        nama: form.nama.trim(),
  ...
  async function handleDelete() {
    if (!form.id) return;
    const confirmed = window.confirm(`Nonaktifkan supplier "${form.nama}"?`);
    if (!confirmed) return;
  ...
  return (
    <div>
      <PageHeader
        eyebrow="Master Data"
        title="Supplier"
        description="Kelola supplier dengan pola master data yang sama: company scope, soft delete, dan restore."
        actions={<Button icon={Plus} onClick={openCreateDrawer}>Tambah Supplier</Button>}
      />
      ...
      {loading ? (
        ...
        <EmptyState
          ...
          description="Tambahkan supplier pertama untuk melengkapi master data company ini."
          action={<Button icon={Plus} onClick={openCreateDrawer}>Tambah Supplier</Button>}
        />
      )}

      <Drawer
        description={form.id ? 'Edit detail supplier dan simpan perubahan.' : 'Tambah supplier baru ke master data company aktif.'}
        ...
        footer={
          <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            ...
            <div className="flex gap-2 sm:justify-end">
              ...
              <Button form="supplier-form" type="submit" disabled={saving || !form.nama.trim()}>
      ...
      >
        <form id="supplier-form" onSubmit={handleSubmit}>
          <SupplierForm data={form} onChange={updateForm} />
        </form>
      </Drawer>
    </div>
  );
